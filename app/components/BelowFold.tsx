...
const events = [
  { time: "17:00", title: "Apéro" },
  { time: "18:30", title: "Dinner" },
  { time: "20:00", title: "Cake Cutting" },
  { time: "21:00", title: "Dance Party" },
  { time: "00:00", title: "Last Dance" },
];
...
function IconDance() {
  return (
    <svg width="52" height="56" viewBox="0 0 52 56" fill="none" stroke={c} strokeWidth="1.2" {...s}>
      ...
      {([
        [19, 27, 2.2, 0], [31, 24, 1.6, 0.5], [22, 38, 1.9, 1.0],
        [35, 33, 1.4, 0.3], [16, 36, 1.3, 0.8], [29, 42, 1.5, 1.3],
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button
        ...
        style={{ width: "100%", background: "#fff", border: "1px solid rgba(107,90,69,0.3)", color: "#4a3728", padding: "0.6rem 0.75rem", fontSize: "0.9rem", fontFamily: "'Jost', sans-serif", cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center" }}
      >
        <span>{selected.title}</span>
        <span style={{ color: "#6b5a45", fontSize: "0.7rem" }}>{open ? "▲" : "▼"}</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid rgba(107,90,69,0.3)", borderTop: "none", zIndex: 10, animation: "dropdownOpen 0.2s ease both", boxShadow: "0 4px 16px rgba(107,90,69,0.12)" }}>
          {t.meals.map((meal) => (
            <button
              key={meal.id}
              type="button"
              onClick={() => { onChange(meal.id); setOpen(false); }}
              style={{ width: "100%", textAlign: "left", padding: "0.75rem", background: meal.id === value ? "rgba(107,90,69,0.08)" : "#fff", border: "none", borderBottom: "1px solid rgba(107,90,69,0.1)", cursor: "pointer" }}
    ...
    <>
                ...
                return (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "6rem 2.5rem auto 8rem", alignItems: "center", padding: "0.55rem 0", position: "relative" }}>
                    ...
                    <div style={{ textAlign: "right", paddingRight: "0.75rem" }}>
                      <span style={{ color: "#8a7060", fontFamily: "'Jost', sans-serif", fontSize: "0.85rem", letterSpacing: "0.1em", fontWeight: 300 }}>{event.time}</span>
                    </div>
                    <div />
                    ...
                    <div style={{ paddingLeft: "1rem" }}>
                      <span style={{ color: "#4a3728", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "1.4rem", fontWeight: 300, letterSpacing: "0.05em" }}>{displayTitle}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <section id="gallery" className="scroll-section" style={{ background: "#fdf7f0", paddingTop: "4rem", paddingBottom: "4rem" }}>
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div style={{ marginTop: "3rem", border: "1px solid rgba(107,90,69,0.3)", background: "rgba(107,90,69,0.06)", padding: "4rem 2rem" }}>
            ...
            <p style={{ color: "#8a7060", fontSize: "0.95rem", lineHeight: 1.8, maxWidth: "400px", margin: "0 auto" }}>{t.gallery.description}</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div style={{ border: "1px solid rgba(107,90,69,0.3)", background: "rgba(107,90,69,0.05)", padding: "1.25rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              <button onClick={() => parkingNav(-1)} style={{ flexShrink: 0, background: "rgba(107,90,69,0.1)", border: "1px solid rgba(107,90,69,0.7)", color: "#4a3728", width: 36, height: 36, cursor: "pointer", fontSize: "1.2rem", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 400 }}>‹</button>
              <div onMouseEnter={stopParkingTimer} onTouchStart={stopParkingTimer} style={{ flex: 1, overflow: "hidden", border: "1px solid rgba(107,90,69,0.2)", aspectRatio: "4/3", position: "relative" }}>
                <iframe key={parkingIdx} src={`https://www.google.com/maps?q=${PARKING[parkingIdx].lat},${PARKING[parkingIdx].lng}&z=17&output=embed`} width="100%" height="100%" style={{ border: 0, display: "block", pointerEvents: "none" }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
            ...
            <div style={{ marginTop: "0.75rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <p style={{ color: "#4a3728", fontFamily: "'Jost', sans-serif", fontSize: "0.9rem", fontWeight: 300, marginBottom: "0.1rem" }}>{PARKING[parkingIdx].name}</p>
                <p style={{ color: "#8a7060", fontFamily: "'Jost', sans-serif", fontSize: "0.78rem", fontWeight: 200, letterSpacing: "0.05em" }}>🚶 {PARKING[parkingIdx].distance}</p>
              </div>
              <div style={{ display: "flex", gap: "0.4rem" }}>
                {PARKING.map((_, i) => (
                  <button key={i} onClick={() => { stopParkingTimer(); setParkingIdx(i); }} style={{ width: 7, height: 7, borderRadius: "50%", background: "#6b5a45", opacity: i === parkingIdx ? 0.8 : 0.2, border: "none", cursor: "pointer", padding: 0 }} />
                ))}
              </div>
            </div>
          </div>
      ...
      <section id="gifts" className="scroll-section" style={{ background: "#fdf7f0", paddingTop: "4rem", paddingBottom: "6rem" }}>
        <div className="max-w-2xl mx-auto px-6 text-center">
          ...
          <div style={{ marginTop: "3rem" }}>
            ...
            <p style={{ color: "#4a3728", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(1.1rem, 3vw, 1.3rem)", fontStyle: "italic", fontWeight: 300, lineHeight: 1.8, maxWidth: "420px", margin: "0 auto 2rem" }}>{t.gifts.text}</p>
            <a href="https://revolut.me/messi_18122022" target="_blank" rel="noopener noreferrer"
              style={{ display: "inline-block", padding: "0.9rem 2.5rem", border: "1px solid #6b5a45", color: "#6b5a45", fontFamily: "'Jost', sans-serif", fontSize: "0.85rem", fontWeight: 400, letterSpacing: "0.2em", textTransform: "uppercase", textDecoration: "none", transition: "background 0.2s, color 0.2s" }}
            ...
            >{t.gifts.button}</a>
            <p style={{ color: "#8a7060", fontFamily: "'Jost', sans-serif", fontSize: "0.75rem", fontWeight: 200, marginTop: "1rem", letterSpacing: "0.05em" }}>{t.gifts.note}</p>
          </div>
        </div>
      </section>
      ...
      <section id="note" className="scroll-section" style={{ background: "#fdf7f0", paddingTop: "4rem", paddingBottom: "6rem" }}>
        <div className="max-w-xl mx-auto px-6 text-center">
          <h2 style={{ color: "#6b5a45", fontFamily: "'Jost', sans-serif", fontSize: "clamp(2rem, 5vw, 3rem)", fontWeight: 300, letterSpacing: "0.1em", marginBottom: "2rem" }}>{t.note.title}</h2>
          <p style={{ color: "#4a3728", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(1.15rem, 3vw, 1.4rem)", fontWeight: 300, lineHeight: 1.9, fontStyle: "italic", marginBottom: "1.5rem" }}>{t.note.text}</p>
          <p style={{ color: "#6b5a45", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "1.3rem", fontWeight: 300, marginTop: "2rem", letterSpacing: "0.08em" }}>— Sarah &amp; Ihssan</p>
        </div>
      </section>
    </>
  );
